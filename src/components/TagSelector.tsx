...
export const TagSelector: React.FC<TagSelectorProps> = ({
  ...
  const handleCreateTag = () => {
    ...
    setNewTag({ label: '', color: '#0A1F3B', category: 'custom' });
    setIsCreating(false);
  };
    ...
    return (
      <div className="p-4 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            <label htmlFor="new-tag-category" className="via-eyebrow">Categoria</label>
            <select
              id="new-tag-category"
              value={newTag.category}
              onChange={(e) => setNewTag({ ...newTag, category: e.target.value })}
              className="w-full h-9 px-3 rounded-md bg-secondary border border-input text-sm text-foreground"
            >
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {Object.entries(tagsByCategory).map(([category, tags]) => (
          <div key={category} className="mb-3">
            <h5 className="via-eyebrow text-muted-foreground px-2 py-1.5">
